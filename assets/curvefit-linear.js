...
function curvefit(str, form) {
	...
	XY = ss.map(u => [+u[0], +u[1], 1.0]);
	Z  = ss.map(u => +u[2]);
    }
    else {
	ss = G29W.map(u => u.match(/[-+.0-9]+/g));
	XY = ss.map(u => [+u[4], +u[5], 1.0]);
	Z  = ss.map(u => +u[3]);
	if (ss[0].length < 6)
	    // x, y are missing, so guess using mesh indices
	    XY = ss.map(u => [grid(+u[1]), grid(+u[2]), 1.0]);
    }
    ...
    function z(x, y) {
        return math.multiply(FIT, math.transpose([ x, y, 1.0 ]));
    }
    ...
    s('; ');
    s('; bilinear bed level mesh, least-square fit to a plane');
    s('; z = $0 x + $1 y + $2', FIT, {notation:'auto', precision:6});
